...
export function categorizeMovimentacao(descricao, tipo, valor = 0) {
  ...
  if (desc.match(/JADLOG|TOTAL EXPRESS|AZUL CARGO|LOGGI|MELHOR ENVIO/i)) {
    return {
      categoria: 'Logística',
  ...
  // ===== RECEITAS - VENDAS ONLINE =====
  if (desc.match(/VINDI|APPMAX|YAMPI|CARTPANDA|HOTMART|EDUZZ|KIWIFY|MONETIZZE/i) && tipo === 'entrada') {
    return {
      categoria: 'Receitas',
