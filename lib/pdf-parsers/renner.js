...
function extrairTotalFaturaPDF(texto) {
  // Padrão 1: "Pagamento Total" seguido de valor (formato do resumo)
  // No texto extraído: "Pagamento Total Limite Total" ... "5.046,18"
  // Ou diretamente: "Pagamento Total\n5.046,18"
  const regexPagTotal = /PAGAMENTO\s+TOTAL[\s\S]*?(\d{1,3}(?:\.\d{3})*,\d{2})/i;
  const match = regexPagTotal.exec(texto);
  if (match) {
    const valor = parseValorBR(match[1]);
    if (valor > 0) {
      console.log(`[Renner Parser] Total fatura extraído via "Pagamento Total": ${valor}`);
      return valor;
    }
  }

  // Padrão 2: "Total R$" seguido de valor (resumo da fatura na pág. 2)
  const regexTotal = /TOTAL\s+R\$\s*(\d{1,3}(?:\.\d{3})*,\d{2})/i;
  const matchTotal = regexTotal.exec(texto);
  if (matchTotal) {
    const valor = parseValorBR(matchTotal[1]);
    if (valor > 0) {
      console.log(`[Renner Parser] Total fatura extraído via "Total R$": ${valor}`);
      return valor;
    }
  }
...
function classificarTipoRenner(descricaoCompleta) {
  ...
  // Pagamentos do cliente (IGNORAR — não incluir no resultado)
...
export function parseRenner(texto) {
  ...
  const confianca = transacoes.length >= 3 ? 'alta' : 'baixa';
