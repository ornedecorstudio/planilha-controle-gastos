...
function extrairTotalFaturaPDF(texto) {
  // IMPORTANTE: No pdf-parse, o layout em caixa da página 1 faz o valor "5.046,18"
  // aparecer ANTES do label "Pagamento Total" no texto extraído:
  //   "R$ 797,56\n5.046,18\nAlternativas complementares de pagamento\nPagamento Total"
  // Por isso o padrão prioritário busca o valor ANTES de "Pagamento Total".

  // Padrão 1 (prioritário): valor que aparece ANTES de "Pagamento Total" no texto
  const regexAntes = /(\d{1,3}(?:\.\d{3})*,\d{2})\s*\n[^\n]*\nPagamento\s+Total/i;
  const matchAntes = regexAntes.exec(texto);
  if (matchAntes) {
    const valor = parseValorBR(matchAntes[1]);
    if (valor >= 100) {
      console.log(`[Renner Parser] Total fatura extraído (antes do label): ${valor}`);
      return valor;
    }
  }

  // Padrão 2: valor >= 1000 próximo (antes) de "Pagamento Total" — threshold alto
  // para evitar capturar "797,56" (Mínimo) que também aparece antes no texto
  ...
  if (matchPerto) {
    ...
    if (valor >= 1000) {
      console.log(`[Renner Parser] Total fatura extraído (próximo ao label): ${valor}`);
      return valor;
    }
  }

  // Padrão 3: "Total R$ VALOR" na seção de resumo da página 2
  ...
  if (matchTotalRS) {
    ...
    if (valor >= 100) {
      console.log(`[Renner Parser] Total fatura extraído (resumo página 2): ${valor}`);
      return valor;
    }
  }

  // Padrão 4 (fallback): "Pagamento Total" seguido de valor (caso texto venha em ordem normal)
  ...
  if (matchDepois) {
    ...
    if (valor >= 100) {
      console.log(`[Renner Parser] Total fatura extraído (após label): ${valor}`);
      return valor;
    }
  }
