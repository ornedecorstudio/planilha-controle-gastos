 ...
 *
 * CUIDADO: Faturas Itaú incluem seção de "Parcelas fixas" com simulação
 * de parcelamento (ex: 10x com juros). O "Total a pagar" dessa seção
 * é MAIOR que o total real da fatura (inclui juros, IOF financiamento).
 * Por isso, NÃO podemos simplesmente pegar o maior valor.
 *
 * Estratégia:
 *   1. Buscar "total desta fatura" (padrão mais específico do Itaú)
 *   2. Buscar "total da fatura" / "total a pagar" FORA de contexto de parcelamento
 *   3. Retornar o PRIMEIRO match válido (não o maior)
 */
function extrairTotalFaturaPDF(texto) {
  const textoUpper = texto.toUpperCase();

  // Contextos que indicam seção de parcelamento/financiamento (NÃO são o total real)
  ...
  /**
   * Verifica se a posição do match está dentro de um contexto de parcelamento.
  ...
  // 1. Padrão prioritário: "total desta fatura" (mais específico do Itaú)
  ...
  if (match) {
    ...
    if (valor > 0) {
      console.log(`[Itaú Parser] Total extraído via "total desta fatura": ${valor}`);
      return valor;
    }
  }

  // 2. Padrão Itaú: "O total da sua fatura é R$ XX.XXX,XX"
  const regexFrase = /(?:total\s+da\s+sua\s+fatura\s+[eé]\s*)R?\$?\s*(\d{1,3}(?:\.\d{3})*,\d{2})/gi;
  match = regexFrase.exec(texto);
  if (match) {
    const valor = parseValorBR(match[1]);
    if (valor > 0) {
      console.log(`[Itaú Parser] Total extraído via "total da sua fatura é": ${valor}`);
      return valor;
    }
  }

  // 3. Padrões genéricos, filtrando contexto de parcelamento
  const regexGenerico = [
  ...
  for (const regex of regexGenerico) {
    ...
    while ((m = regex.exec(texto)) !== null) {
      if (!emContextoParcelamento(m.index)) {
        ...
        if (valor > 0) {
          console.log(`[Itaú Parser] Total extraído via padrão genérico (fora de parcelamento): ${valor}`);
          return valor;
        }
      }
    }
  ...
  // 4. Nenhum match confiável encontrado
  console.log('[Itaú Parser] Nenhum total da fatura encontrado fora de contexto de parcelamento');
  return null;
}
...
function extrairSubtotais(texto) {
  ...
  // Contextos que indicam seção de parcelamento/financiamento (não são subtotais reais)
  const contextosParcelamento = [
    'PARCELA', 'JUROS', 'FINANC', 'CET ', 'A.M.', 'FIXAS',
    'CREDICARD', 'PARCELAMENTO', 'IOF FINANC', 'TAXA DE JUROS',
    'ENCARGOS', 'EM AT'
  ...
  while ((match = regex.exec(texto)) !== null) {
    // Filtrar subtotais de seções de parcelamento/financiamento
    if (emContextoParcelamento(match.index)) {
      console.log(`[Itaú Parser] Subtotal ignorado (contexto parcelamento): ${match[0].trim()}`);
      continue;
    }
