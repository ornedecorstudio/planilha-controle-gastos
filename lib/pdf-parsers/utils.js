...
export function calcularAuditoria(transacoes, totalFaturaPDF = null, resumoFatura = null) {
  ...
  const pagamentoFatura = pagamentoFaturaTransacoes > 0
  ...
  const totalFaturaCalculado = parseFloat(
    (totalCompras + iof + tarifaCartao + saldoAnterior + juros + multas
     - pagamentoFatura - estornos - pagamentoAntecipado).toFixed(2)
  );
  ...
  equacao += ` - ${estornos.toFixed(2)} - ${pagamentoAntecipado.toFixed(2)} = ${totalFaturaCalculado.toFixed(2)}`;
