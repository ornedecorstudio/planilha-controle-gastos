...
function extrairTotalFaturaPDF(texto) {
  ...
  // 1. Padrao prioritario: "Total da fatura" (resumo na pag. 1)
  ...
  while ((match = regexTotal.exec(texto)) !== null) {
    if (!emContextoParcelamento(textoUpper, match.index)) {
      ...
      if (valor > 0) {
        console.log(`[PicPay Pipeline] Total extraido via "total da fatura": ${valor}`);
        return valor;
      }
    }
  }
  ...
  while ((match = regexPagTotal.exec(texto)) !== null) {
    if (!emContextoParcelamento(textoUpper, match.index)) {
      const valor = parseValorBR(match[1]);
      if (valor > 0) {
        console.log(`[PicPay Pipeline] Total extraido via "pagamento total": ${valor}`);
        return valor;
      }
    }
  }

  // 4. Generico: "Total a pagar" fora de contexto
  const regexTotalPagar = /TOTAL\s+(?:A\s+)?PAGAR\s*:?\s*R?\$?\s*(\d{1,3}(?:\.\d{3})*,\d{2})/gi;
  while ((match = regexTotalPagar.exec(texto)) !== null) {
    if (!emContextoParcelamento(textoUpper, match.index)) {
      const valor = parseValorBR(match[1]);
      if (valor > 0) {
        console.log(`[PicPay Pipeline] Total extraido via "total a pagar": ${valor}`);
        return valor;
      }
    }
  }
...
ATENCAO — LAYOUT DUAS COLUNAS:
Este PDF PicPay tem layout de DUAS COLUNAS lado a lado (um cartao a esquerda, outro a direita).
Leia AMBAS as colunas de TODAS as paginas. Nao pule nenhuma coluna.
Sao aproximadamente 200+ transacoes distribuidas em 7-8 paginas.
${metadadosBloco}

REGRAS DE EXTRACAO — LEIA COM MUITA ATENCAO:

1. EXTRAIA TODAS as transacoes de TODOS os cartoes presentes no PDF (sao ${cartoesInfo ? metadados.cartoes.length : 'varios'} cartoes)
2. Percorra TODAS as paginas de 1 a 8 (paginas 9-10 sao informacoes financeiras — IGNORE)
3. Para cada pagina, leia AMBAS as colunas (esquerda e direita)
4. Cada cartao tem secoes "Transacoes Nacionais" e possivelmente "Transacoes Internacionais"

TRANSACOES INTERNACIONAIS — VALOR BRL:
- Transacoes internacionais mostram valor em USD E valor convertido em BRL
- Use SEMPRE o valor em BRL (o maior valor, ja convertido), NUNCA o valor em USD
- Exemplo: "Dolar: 72,32 | Cambio do dia: R$ 5,7918 | 72,32  418,86" -> use 418,86 (BRL)
...
VALORES NEGATIVOS:
- Valores com sinal negativo (-) no PDF sao estornos/creditos
- Capture-os com tipo_lancamento "estorno" e valor POSITIVO no JSON

IGNORE completamente (NAO inclua no JSON):
- "PAGAMENTO DE FATURA PELO PICPA" ou qualquer variacao (e o pagamento da fatura anterior)
- "Pagamento recebido", "Pagamento efetuado"
- Linhas de "Subtotal dos lancamentos", "Total geral dos lancamentos"
- Cabecalhos de secoes e titulos de cartoes
- Informacoes de milhas Smiles (ex: "12345 milhas")
- Informacoes financeiras das paginas 9-10 (parcelamento, juros, CET, IOF financiamento)

FORMATO:
- Data: DD/MM/YYYY (adicione o ano baseado no vencimento da fatura)
- Valor: numero positivo com 2 casas decimais (ex: 1234.56, NAO 1.234,56)
...
Se a soma ficar muito diferente, revise se nao esqueceu transacoes de alguma coluna ou pagina.
