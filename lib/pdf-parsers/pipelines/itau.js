...
CONTEXTO IMPORTANTE:
Esta fatura Itau tem layout de DUAS COLUNAS. ${totalFatura}${subtotaisInfo}${cartoesInfo}

REGRAS DE EXTRACAO — LEIA COM ATENCAO:
1. EXTRAIA TODAS as transacoes de TODOS os cartoes presentes no PDF
2. Inclua transacoes de TODAS as secoes: "compras e saques", "transacoes internacionais", "outros lancamentos"
3. Para transacoes internacionais, use SEMPRE o valor ja convertido em BRL (nao o valor em moeda estrangeira)
4. NAO duplique transacoes
5. Data deve estar no formato DD/MM/YYYY (adicione o ano baseado no vencimento da fatura)
6. Valor deve ser numero positivo (ex: 1234.56)

CLASSIFICACAO tipo_lancamento — cada transacao DEVE ter um tipo_lancamento:
...
- "Pagamento fatura", "Pagamento recebido", "Pagamento efetuado" (sao pagamentos do cliente)
- Linhas de subtotal, total, saldo anterior
- Cabecalhos de secoes

VERIFICACAO: a soma de TODAS as transacoes tipo "compra" + "iof" + "tarifa_cartao" - "estorno" - "pagamento_antecipado" deve ser proxima de ${metadados?.total_fatura_pdf ? `R$ ${metadados.total_fatura_pdf.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : 'o total da fatura no PDF'}.
