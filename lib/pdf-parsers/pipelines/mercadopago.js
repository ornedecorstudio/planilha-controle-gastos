...
function extrairResumoFatura(texto) {
  ...
  // --- Total a pagar: encoding LIMPO (ultima linha do resumo) ---
  // Este campo usa fonte nao-corrompida no PDF Mercado Pago
  const matchTotal = texto.match(/Total\s+R\$\s*(\d{1,3}(?:\.\d{3})*,\d{2})/);
  if (matchTotal) {
    resumo.total_a_pagar = parseValorBR(matchTotal[1]);
    camposExtraidos++;
  }
...
export function extractPipeline(texto, options = {}) {
  ...
  //
  // Prioridade: resumo.total_a_pagar > totalBruto.bruto > null
  // (se o resumo nao extraiu o total, cai no bruto como fallback parcial)
