...
function extrairTotalBrutoFatura(texto) {
  ...
  if (totalConsumos === 0) {
    const regexConsumosCorrupt = /[ií]onsumos?\s+(?:de\s+)?\d{1,2}\/\d{1,2}\s+a\s+\d{1,2}\/\d{1,2}[\s\S]{0,80}?\$4\s+([\d,J%\+\)\(]+)/i;
  ...
  if (totalTarifas === 0) {
    const regexTarifasCorrupt = /Tarifas?\s+e\s+encargos?[\s\S]{0,40}?\$4\s+([\d,J%\+\)\(]+)/i;
...
export function extractPipeline(texto, options = {}) {
  ...
  // Para reconciliacao, SEMPRE preferir bruto (comparavel com soma das transacoes).
  // O liquido (Total a pagar) inclui pagamentos anteriores e NAO bate com a soma.
  //
  // FIX v2: Adicionado log de warning quando cai no fallback, para diagnostico.
  // Se o bruto falhar, o fallback para liquido VAI causar divergencia na reconciliacao
  // (a diferenca sera exatamente o valor dos pagamentos anteriores - saldo anterior).
  const totalParaReconciliacao = totalBruto?.bruto || null;

  if (totalParaReconciliacao) {
    console.log(`[Mercado Pago Pipeline] Total para reconciliacao (BRUTO): ${totalParaReconciliacao}`);
  } else {
    // FIX v2: NAO usar liquido como fallback para reconciliacao.
    // Se o bruto nao foi extraido, deixar null — a IA vai calcular via campo total_a_pagar.
    console.warn(`[Mercado Pago Pipeline] AVISO: Nao foi possivel extrair total bruto (Consumos + Tarifas) do texto corrompido.`);
    console.warn(`[Mercado Pago Pipeline] Total liquido (informativo): ${totalLiquido}`);
  ...
  return {
    ...
    metadados_verificacao: {
      total_fatura_pdf: totalParaReconciliacao,   // FIX v2: bruto puro, sem fallback para liquido
      total_liquido_pdf: totalLiquido,             // Mantido separado para exibicao informativa
...
IMPORTANTE: O "Total a pagar" e MENOR que o bruto porque inclui creditos de pagamentos
anteriores que o cliente fez. A diferenca tipica e o valor de overpayment do ciclo anterior.
Exemplo: Se o cliente pagou R$ 180,04 a mais na fatura passada, o "Total a pagar" sera
R$ 180,04 menor que o bruto. Use SEMPRE o bruto para reconciliacao.
