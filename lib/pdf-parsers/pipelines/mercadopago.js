 ...
 *   - "Consumos de DD/MM a DD/MM R$ 12.979,63" (total bruto das compras)
 *   - "Tarifas e encargos R$ 14,90"
 *
...
function extrairTotalBrutoFatura(texto) {
  // Padrao: "Consumos de DD/MM a DD/MM R$ 12.979,63"
  const regexConsumos = /Consumos?\s+(?:de\s+)?\d{1,2}\/\d{1,2}\s+a\s+\d{1,2}\/\d{1,2}\s+R?\$?\s*(\d{1,3}(?:\.\d{3})*,\d{2})/i;
  const matchConsumos = regexConsumos.exec(texto);

  let totalConsumos = 0;
  if (matchConsumos) {
    totalConsumos = parseValorBR(matchConsumos[1]);
  }

  // Padrao: "Tarifas e encargos R$ 14,90"
  const regexTarifas = /Tarifas?\s+e\s+encargos?\s+R?\$?\s*(\d{1,3}(?:\.\d{3})*,\d{2})/i;
  const matchTarifas = regexTarifas.exec(texto);

  let totalTarifas = 0;
  if (matchTarifas) {
    totalTarifas = parseValorBR(matchTarifas[1]);
  }

  if (totalConsumos > 0) {
    console.log(`[Mercado Pago Pipeline] Total bruto: consumos=${totalConsumos}, tarifas=${totalTarifas}, bruto=${totalConsumos + totalTarifas}`);
    return {
      consumos: totalConsumos,
      tarifas: totalTarifas,
      bruto: parseFloat((totalConsumos + totalTarifas).toFixed(2))
    };
  }
 ...
 * comparavel com a soma bruta das transacoes. Usar `extrairTotalBrutoFatura()`
 * para reconciliacao. Esta funcao e mantida como fallback.
 *
 * Padroes reconhecidos:
...
export function extractPipeline(texto, options = {}) {
  ...
  // ----- Calculos finais -----
  const totalLiquido = extrairTotalFaturaPDF(texto);      // "Total a pagar" (liquido, inclui creditos)
  const subtotaisBruto = extrairTotalBrutoFatura(texto);   // Consumos + Tarifas (bruto)

  // Para reconciliacao, usar o total bruto (comparavel com soma das transacoes).
  // O "Total a pagar" e LIQUIDO (inclui creditos de pagamentos anteriores) e
  // NAO bate com a soma bruta das transacoes extraidas.
  const totalParaReconciliacao = subtotaisBruto?.bruto || totalLiquido;

  console.log(`[Mercado Pago Pipeline] Total para reconciliacao: ${totalParaReconciliacao} (bruto: ${subtotaisBruto?.bruto}, liquido: ${totalLiquido})`);

  const valorTotalCompras = transacoes
    .filter(t => t.tipo_lancamento === 'compra')
    .reduce((sum, t) => sum + t.valor, 0);

  const auditoria = calcularAuditoria(transacoes, totalParaReconciliacao);
  ...
  return {
    ...
    metadados_verificacao: {
      total_fatura_pdf: totalParaReconciliacao,   // bruto para reconciliacao
      total_liquido_pdf: totalLiquido,             // liquido original (informativo)
      subtotais_bruto: subtotaisBruto,             // detalhamento consumos/tarifas
...
Retorne apenas um JSON valido, sem markdown e sem comentarios:
{
  ...
  "banco_detectado": "Mercado Pago",
  "total_a_pagar": valor_total_bruto_dos_consumos_mais_tarifas_e_encargos
}

O campo total_a_pagar deve ser o valor BRUTO da fatura: soma de "Consumos de DD/MM a DD/MM" + "Tarifas e encargos". NAO use o "Total a pagar" da primeira pagina, pois esse valor e liquido (descontados pagamentos anteriores) e nao serve para reconciliacao.
...
- "total_a_pagar": valor bruto (Consumos + Tarifas e encargos). Esse valor e usado para reconciliacao automatica.
</formato_saida>`;
}
