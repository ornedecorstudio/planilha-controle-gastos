...
ATENCAO — LAYOUT COLUMNAR:
Este PDF Santander tem layout columnar onde datas/descricoes ficam separados dos valores.
NAO confie no texto extraido — leia VISUALMENTE as tabelas do PDF.
${metadadosBloco}

ESTRUTURA DO PDF SANTANDER:
- Pagina 1: RESUMO APENAS — Total a Pagar, Pagamento Minimo, limite de credito, historico, anuidade. NAO contem transacoes de compra.
- Paginas 2-3: "Detalhamento da Fatura" — transacoes organizadas por cartao
- Cada cartao tem um bloco com nome e ultimos 4 digitos (ex: "NOME - 4258 XXXX XXXX 8172")
- Dentro de cada cartao ha secoes: "Despesas", "Parcelamentos", "Pagamento e Demais Creditos"
- "Resumo da Fatura" no final — NAO sao transacoes, e para verificacao

REGRAS DE EXTRACAO:

1. Extraia transacoes APENAS de "Detalhamento da Fatura" (paginas 2-3)
2. Extraia de TODOS os cartoes (${cartoesInfo || 'multiplos cartoes'})
3. Cada transacao tem: Data (DD/MM) | Descricao | Valor em R$
4. Leia os valores VISUALMENTE da coluna "R$" — o texto extraido pode estar corrompido

IGNORE completamente (NAO inclua no JSON):
- TODA a pagina 1 (resumo, opcoes de pagamento, historico, limites de credito)
- "Seu Limite e:" e QUALQUER valor de limite — e o limite do cartao, NAO uma transacao
- "Pagamento Total" e "Pagamento Minimo" da pagina 1 — sao opcoes de pagamento
- "PAGAMENTO DE FATURA-INTERNET" e toda secao "Pagamento e Demais Creditos" — sao pagamentos da fatura anterior, NAO sao despesas
- "Historico de Faturas" e valores de meses anteriores (NOV, DEZ, JAN, FEV)
- "Resumo da Fatura" — e apenas para verificacao, NAO sao transacoes
- Linhas de "VALOR TOTAL" — sao subtotais de secao, NAO transacoes individuais
- Informacoes de Smiles/milhas
...
PARCELAMENTOS:
- Transacoes da secao "Parcelamentos" sao compras parceladas
- Inclua com tipo_lancamento "compra" e campo parcela "X/Y" se disponivel
...
Se a soma ficar muito diferente, revise se nao esqueceu transacoes de algum cartao ou pagina.
